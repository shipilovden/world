...
export default function Avatar() {
  ...
  // Состояние для режима бега
  const [isRunMode, setIsRunMode] = useState(false);
  
  // Состояние для режима следования камеры
  const [cameraFollowMode, setCameraFollowMode] = useState(false);
  
  // Получаем доступ к камере и орбитальному контролю
  const { camera } = useThree();
  const orbitControlsRef = useRef();
  
  // Реф для контроллера персонажа
  const characterControlsRef = useRef();

  // Загружаем GLTF модель и анимации
  const { scene, animations } = useGLTF('/models/Soldier.glb');

  // Подключаем анимации к нужной группе
  const { actions, names, mixer } = useAnimations(animations, group);

  // Добавляем физическую коробку (коллизию) вокруг модели
  const [ref] = useBox(() => ({
    mass: 1,                  // Вес тела
  ...
  // Запускаем Idle-анимацию и настраиваем тени
  useEffect(() => {
  ...
  // Обновляем контроллер персонажа в каждом кадре
